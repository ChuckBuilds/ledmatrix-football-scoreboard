...
When favorite teams are configured and `show_favorite_teams_only: true`, the limits apply **per team**.
...
When no favorite teams are configured or `show_favorite_teams_only: false`, the limits apply to the **total number of games** across all teams.
...
```
**Result:**
- NFL upcoming: 2 games (1 per team × 2 teams)
- NFL recent: 4 games (2 per team × 2 teams)
- NCAA FB upcoming: 3 games total (no favorites, total limit)
- NCAA FB recent: 5 games total (no favorites, total limit)
...
### Duplicate Games
If a game involves multiple favorite teams (e.g., TB vs DAL and both are favorites), the game appears only once in the final list. Duplicates are automatically removed.
